...
//fucntion to read in stream
void readMovesFromFile(const char *file_name, int table_size, char **table, int *move_count, int *count_check) {
    ...
    while (fscanf(file, "%d,%d;", &row, &col) == 2) {
        ...
        // Update the table based on the read moves
        updateTable(table_size, table, row, col, (*move_count)++, *count_check);
    }
    ...
    while (1) {
        if (fgets(buffer, sizeof(buffer), stdin) != NULL) {
            if (sscanf(buffer, "%d", row_num) == 1) {
                if ((*row_num >= 1 && *row_num <= *table_size) || *row_num == 123) {
                    break;
                }
            }
        }
    ...
    while (1) {
        if (fgets(buffer, sizeof(buffer), stdin) != NULL) {
            if (sscanf(buffer, "%d", col_num) == 1) {
                if ((*col_num >= 1 && *col_num <= *table_size) || *col_num == 123) {
                    break;
                }
            }
        }
